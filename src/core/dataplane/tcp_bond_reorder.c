...
#define TCP_BOND_WINDOW           16384u
#define TCP_BOND_DEFAULT_HOLD_NS  (10ULL * 1000000ULL)
...
static struct tcp_bond_stream g_streams[TCP_BOND_STREAMS];
...
static atomic_uint_fast32_t g_tx_seq[TCP_BOND_STREAMS];
                             ...
                             uint32_t *seq)
{
    ...
    if (!epoch || !seq)
        return -1;
    value = (uint32_t)atomic_fetch_add_explicit(&g_tx_seq[wire_policy_id], 1u,
                                                memory_order_relaxed) + 1u;
    if (!value)
        value = (uint32_t)atomic_fetch_add_explicit(&g_tx_seq[wire_policy_id],
                                                    1u,
                                                    memory_order_relaxed) + 1u;
    *epoch = tx_epoch();
    *seq = value;
...
void dp_tcp_bond_reorder_configure_from_env(void)
{
    ...
    if (!g_configured) {
        for (uint32_t i = 0; i < TCP_BOND_STREAMS; i++)
            (void)pthread_spin_init(&g_streams[i].lock,
...
void dp_tcp_bond_reorder_submit(uint8_t wire_policy_id,
                                ...
                                const struct dp_tcp_bond_ops *ops)
{
    struct tcp_bond_stream *stream = &g_streams[wire_policy_id];
    struct tcp_bond_slot *slot;
    int32_t delta;
    int owner = 0;
    int drop_input = 0;

    if (!item || !g_enabled || !g_configured) {
        item_emit(ops, item);
        return;
    }
                            ...
                            const struct dp_tcp_bond_ops *ops)
{
    if (!g_enabled || !g_configured)
        return;
    for (uint32_t i = 0; i < TCP_BOND_STREAMS; i++) {
        struct tcp_bond_stream *stream = &g_streams[i];
        int owner = 0;
